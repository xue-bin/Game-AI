...
public class LevelOptimizer
{
    ...
    const int MAX_ATTEMPTS =100;//The max number of times to iterate the algorithm
    ...
    public Level GetLevel(Evaluator evaluator, float goalValue)
    {
        ...
        */
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++)
        {
            ...
            {
                Level l1 = Sample(population, evaluator, goalValue);
                Level l2 = Sample(population, evaluator, goalValue);
                population.Add(Crossover(l2, l1));
            }
